...
## 9. Prompt final para Sonnet 4.5

```
CONTEXTO
========
Implementação final (V4) da página pública do mestre (/mestre/:slug) em React (Vite + TS + Tailwind) + backend Node/Express/Kysely. Etapas 1–3 (segurança, arquitetura, contrato) já estão concluídas e funcionais.

O objetivo é executar os 21 patches documentados em Reformulacao_mestre-v4.md, sem reescrever o que já funciona.

ESTADO CONHECIDO E IMUTÁVEL
===========================
- Middleware optionalAuth aplicado em GET /api/v1/gm/:slug: PRESERVAR.
- authMiddleware em GET /api/v1/gm/:slug/insights: PRESERVAR.
- viewer_context no payload público: PRESERVAR.
- closed_group, selling_points, features expostos pelo backend: PRESERVAR.
- gmPanel.ts já aceita os novos campos em POST/PUT /profile: PRESERVAR.
- PainelMestrePage.tsx consome metrics_* via /api/v1/gm/tables: PRESERVAR.

NÃO-NEGOCIÁVEIS
===============
- Nenhum fallback inventado. Se dado não existe, componente retorna null.
- Todo componente usa tipos exportados dos hooks (MestrePublicData, InsightMetric, etc).
- Componentes da pasta components/mestre/ usam Lucide, não emoji.
- CSS adicionado ao MestrePage.css existente, não em arquivos novos.
- Volume compartilhado para Open Graph não pode quebrar deploy existente.

ORDEM OBRIGATÓRIA
=================
Seguir rigorosamente a seção 7 (Plano de execução) do documento Reformulacao_mestre-v4.md. Cada passo tem validação antes do próximo.

Passo 1 — criar og-default.png (1200x630)
Passo 2 — Patch 1 (backend gm.ts buildRecommendations)
Passo 3 — Patch 2 (hook useMestreInsights)
Passo 4 — Patches 6, 7, 8, 9, 11 (novos componentes)
Passo 5 — Patches 5, 3, 4, 12, 17, 10, 15 (reescritas)
Passo 6 — Patches 13, 14 (TableCard, LinksDisplay)
Passo 7 — Patch 16 (CSS)
Passo 8 — remover MestreWhySection.tsx
Passo 9 — Patches 18, 19, 20, 21 (Open Graph + Nginx + docker + index.html)
Passo 10 — EditGmProfileForm.tsx + integração no PainelMestrePage

ANTES DE EXECUTAR CADA PASSO
============================
1. Ler o patch correspondente no documento.
2. Verificar se o arquivo de destino existe e confere com o diagnóstico.
3. Aplicar mudança.
4. Rodar `npm run build` no frontend ou `tsc --noEmit` no backend para validar.
5. Só passar para próximo passo quando compila.

SE ALGO DIVERGE DO DIAGNÓSTICO
===============================
- Parar e perguntar. Não improvisar.
- Exemplo: se o Nginx real do container frontend tem estrutura diferente da assumida, parar e pedir nginx.conf antes de editar.

INFORMAÇÕES NECESSÁRIAS DO USUÁRIO ANTES DO PASSO 9
===================================================
1. Domínio de produção exato (vou assumir https://mesas.artificiorpg.com).
2. Nome do service Docker do backend (para o proxy_pass do Nginx).
3. nginx.conf real do container frontend.
4. Como/onde o backend expõe app.ts ou equivalente para registrar o router OG.
5. Se existe docker-compose combinado frontend+backend ou são arquivos separados.

VALIDAÇÃO FINAL
===============
Rodar o checklist da seção 8 do Reformulacao_mestre-v4.md item a item. Abrir PR apenas quando TODOS os itens passarem.
```
