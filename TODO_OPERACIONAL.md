...
| REQ-09 | 5/4/4 | **Selos oficiais Covil do Lich + DDAL (Fase 2/3):** Implementar política de atribuição e exibição dos selos no backend/frontend. `covil_do_lich` para mestres/mesas da parceria; `ddal` somente para mesa vinculada ao caminho `D&D > D&D 5e > D&D 2024`, com validação obrigatória de `codigo`, `nome` e `tier`. | Em validação beta | Backend e frontend implementados localmente (persistência, filtro `seal`, badges e metadados DDAL). Desbloqueio do QA E2E depende de: migration_02 aplicada no banco beta (feita) + `systemsTreeImport` executado via docker cp pós-deploy (pendente). |
...
| REQ-11 | 4/5/4 | **Papel do publicador da mesa (anunciante vs mestre):** Quem publica uma mesa pode ser o próprio mestre que vai narrá-la, ou apenas um anunciante divulgando a mesa de outro mestre. O campo `publisher_role` (enum: `gm` \| `announcer`) deve ser adicionado à tabela `tables`. Quando `publisher_role = 'announcer'`, o campo `actual_gm_name TEXT` (nome do mestre real, texto livre) deve ser preenchido. **Selo visual obrigatório:** quando `publisher_role = 'announcer'`, exibir um selo discreto e bem posicionado com o texto "Apenas Anunciante" no card do catálogo (canto ou rodapé do card) e na página da mesa (próximo ao nome do publicador). O selo deve comunicar claramente que o publicador não é o mestre que vai narrar — sem julgamento de valor, apenas informativo. Estilo sugerido: badge neutro (cor cinza/secundária, ícone de megafone ou similar), diferente dos selos de parceria (Covil, DDAL) que têm cor e destaque próprios. | Em aberto | **Impacto de schema:** nova coluna `publisher_role` + `actual_gm_name` em `tables` via migration. Não confundir com `gm_id` (quem tem conta e publicou). Ver `ARQUITETURA_PROJETO.md` seção 4 antes de implementar. |
| REQ-12 | 5/5/5 | **Canais de contato e recrutamento obrigatórios na mesa:** Toda mesa deve ter ao menos um canal de recrutamento/contato preenchido. Os canais suportados são: WhatsApp (número/link), Discord (link de convite), Telefone, E-mail, Facebook (link), Instagram (handle/link), Formulário/outro link genérico. **Modelo de dados:** tabela `table_contacts` com colunas `id UUID PK`, `table_id UUID FK tables(id) ON DELETE CASCADE`, `channel TEXT NOT NULL` (enum de valores: `whatsapp`, `discord`, `phone`, `email`, `facebook`, `instagram`, `form`), `value TEXT NOT NULL` (número, link ou handle), `label TEXT` (rótulo opcional exibido ao usuário), `sort_order SMALLINT DEFAULT 0`. **Regra de negócio:** backend rejeita publicação (`status = active`) de mesa sem ao menos um `table_contacts` cadastrado. Frontend exibe os canais na página da mesa e no formulário do painel do mestre. | Em aberto | **Impacto de schema:** nova tabela `table_contacts` via migration. Não usar `source_url` existente — semântica diferente (AggregatorBot). Validar no backend que `value` não está vazio antes de persistir. Não expor telefone/email diretamente no HTML público sem ofuscação ou confirmação do mestre. |
