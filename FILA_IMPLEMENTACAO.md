...
| 129 | Fase 4 | backend | 5/5/5 | Normalização - Priorizar enrichedFields | Atualizar normalizeExporterPayload.ts para priorizar enrichedFields sobre fallback TypeScript. Merge correto: { ...rawPayload, ...enrichedFields } (enrichedFields por último). Preservar attachments, embeds, mentions, author e todos os campos enriquecidos. Logar quais campos vieram do parser e quais ficaram vazios. | backend/src/domain/aggregator/normalizeExporterPayload.ts | pendente | Regra crítica: não há perda silenciosa de campos. parsed_json final contém tudo que o parser devolveu. Ver erro E122. |
| 130 | Fase 4 | backend | 4/5/4 | parseExporterMessage - Priorizar enrichedFields | Atualizar parseExporterMessage.ts para priorizar enrichedFields em todos os campos que existirem. Manter fallback regex apenas quando campo estiver ausente. Não sobrescrever setting_name e setting_styles do parser com parsing fraco. Separar masterText e recruiterName. Determinar is_paid, billing_text, is_ongoing, candidate_kind, recruitment_channels preliminares, requirements preliminares. | backend/src/domain/aggregator/parseExporterMessage.ts | pendente | O TypeScript não deve mais ignorar o Python. Candidato sai com dados suficientes para revisão. |
| 131 | Fase 4 | frontend | 5/5/5 | candidateToFormData - Auto-preenchimento completo | Expandir candidateToFormData.ts para mapear todos os campos do parser: sistema pré-selecionado via system_path_slug, cobrança marcada quando is_paid=true, valor preenchido via priceText, canal Discord criado automaticamente quando origem for Discord, formulário/WhatsApp adicionados quando detectados em external_links/signupText, banner importado via banner_url, avatar do autor via avatar_url, requisitos técnicos marcados (requires_pc/camera/microphone), mesa em andamento marcada (is_ongoing), cenário e estilos preenchidos (setting_name, setting_styles), agenda aproveitada (texto bruto ou estruturada via sessions[]). | frontend/src/utils/candidateToFormData.ts | pendente | Regra: priorizar enrichedFields; fallback para parsing secundário só quando necessário. Ver erros E123, E124. |
| 132 | Fase 4 | frontend | 4/5/4 | Abertura automática de blocos no formulário de revisão | Atualizar formulário de revisão para abrir automaticamente blocos relevantes: bloco de valor quando is_paid=true, preview de banner quando banner_url existir, canais de recrutamento quando detectados, requisitos técnicos quando marcados, checkbox de mesa em andamento, bloco de cenário e estilos quando preenchidos, repetidor de agenda (se suportado) ou texto bruto. Admin pode editar qualquer campo antes de aprovar. | frontend/src/pages/GestaoPage.tsx, frontend/src/pages/PainelMestrePage.tsx | pendente | Sistema não exige redigitação do que já estava no JSON. UX: blocos expandidos por padrão quando contêm dados. |
| 133 | Fase 4 | backend | 5/5/5 | Endpoint accept com overrides opcionais | Modificar endpoint PATCH /api/v1/aggregator/candidates/:id/accept para aceitar body opcional com overrides. Validar apenas campos conhecidos (whitelist). Sanitizar arrays e strings. Passar overrides para candidateService.acceptCandidate(candidateId, overrides). Merge de overrides com parsed_json no service: const finalData = { ...candidate.parsed_json, ...overrides }. | backend/src/routes/aggregatorReview.ts, backend/src/services/aggregator/candidateService.ts | pendente | Backend persiste dados revisados (não apenas brutos). Ver erros E125, E126. |
| 134 | Fase 4 | backend | 5/5/5 | candidateService - Persistir todos os campos revisados | Atualizar candidateService.acceptCandidate() para ler setting_name, setting_styles, banner_url, billing_text, publisher_role, master_display_name, canais de recrutamento, requisitos técnicos, status de andamento de parsed_json ou overrides. Adicionar campos ao INSERT de tables. Validar persistência com query direta no banco após aprovação. | backend/src/services/aggregator/candidateService.ts | pendente | Regra: cenário/estilos/banner/cobrança/recrutamento/requisitos chegam até a tabela tables final. Mesa importada publicada fica indistinguível de mesa manual. |
| 135 | Fase 4 | frontend | 4/4/4 | MesaPage - Renderizar campos importados | Atualizar MesaPage.tsx para renderizar todos os campos importados: cenário e estilos, banner, requisitos técnicos, status de andamento, canais de recrutamento. Garantir que mesa importada seja visualmente indistinguível de mesa criada manualmente. | frontend/src/pages/MesaPage.tsx | pendente | Validação final: mesa importada aprovada deve ter mesma qualidade visual de mesa manual. |
| 136 | Fase 4 | fullstack | 5/5/5 | Testes E2E do fluxo completo | Criar teste E2E que valida fluxo completo: (1) Importar JSON com todos os campos; (2) Parser extrai dados; (3) Normalização preserva campos; (4) Formulário de revisão pré-preenchido; (5) Blocos abertos automaticamente; (6) Admin edita campo; (7) Aprovação persiste override; (8) Mesa publicada exibe todos os dados. | backend/tests/e2e/importacao-inteligente.test.ts | pendente | Teste crítico para garantir que não há perda de dados em nenhuma camada. |
