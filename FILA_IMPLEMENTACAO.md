...
| 058 | Fase 4 | frontend/backend | 4/5/4 | Botão "Desfazer rejeição" | Permitir que admin reverta rejeição de candidato, mudando status de volta para "awaiting_review". Botão visível apenas em filtro "Rejeitadas". Viola H3 (Controle e liberdade). | `frontend/src/pages/GestaoPage.tsx` | concluido | **Score GUT: 80 (4×5×4).** REQ-19. Implementado e deployado em 05/04/2026 (commit a4dc87f): botão "Desfazer" (amarelo) adicionado no frontend. |
...
| ID | Fase | Tipo | GUT | Titulo | Descrição objetiva | Arquivos esperados | Status | Observação |
|---|---|---|---|---|---|---|---|---|
| 141 | Fase 3 | frontend | 5/5/5 | **[BUG CRÍTICO] Corrigir: clicar em "Editar Mesa" abre página vazia** | Ao clicar no botão de editar uma mesa existente, o formulário abre sem nenhum dado pré-preenchido — página vazia. Investigar como a rota de edição carrega dados: (1) Verificar se `GET /api/v1/tables/:id` está sendo chamado; (2) Verificar se o `initialData` está chegando corretamente ao `CreateTableForm`; (3) Verificar se a rota do frontend para edição usa o ID/slug correto; (4) Verificar se o estado de edição no `PainelMestrePage.tsx` está sendo populado antes de renderizar o formulário. | `frontend/src/pages/PainelMestrePage.tsx`, `frontend/src/features/create-table/`, `frontend/src/hooks/useTableData.ts` | concluido — 13/04/2026 | **Score GUT: 125 (5×5×5).** Solução em 2 etapas: (1) 12/04 - mapper `mapTableApiToInitialData.ts` criado + `handleEdit` corrigido em `uiHelpers.ts`; (2) 13/04 - race condition corrigido (item 152) — verificação de `?edit=` antes de forçar dashboard. Commit 8bb716b. Build ✓. |
| 142 | Fase 3 | backend+frontend | 5/5/5 | **[BUG CRÍTICO] Corrigir: erro "token inválido ou expirado" ao desativar mesa** | Ao tentar desativar uma mesa no painel do mestre, aparece erro de token inválido/expirado mesmo com o usuário logado. Investigar: (1) Se o endpoint `PATCH /api/v1/tables/:id/toggle-status` (ou equivalente) está sendo chamado com o header `Authorization: Bearer <token>` correto; (2) Se o middleware de autenticação está rejeitando o token por algum motivo pontual (ex: clock skew, campo faltando); (3) Se há diferença entre a requisição de desativar e outras que funcionam; (4) Verificar se `AuthContext` está enviando token atualizado. Documentar causa raiz em `ERRORS_SOLUTIONS.md`. | `frontend/src/pages/PainelMestrePage.tsx`, `backend/src/middleware/auth.ts`, `backend/src/routes/gmPanel.ts` | concluido — 13/04/2026 | **Score GUT: 125 (5×5×5).** Causa raiz: Frontend usava `PUT /api/v1/gm/tables/:id` que exige todos os campos obrigatórios, ao invés de `PATCH /api/v1/gm/tables/:id/status` que aceita apenas status. Solução: corrigir endpoint e método em `handleToggleTableStatus`. Documentado em E142. Build ✓. |
| 143 | Fase 3 | banco+backend | 4/5/4 | Adicionar campo `name_pt` em sistemas e cenários | Criar campo `name_pt TEXT` (nullable) nas tabelas `systems` e `scenarios` para armazenar o nome em português do sistema/cenário. Exemplos: sistema `Dungeons & Dragons` → `name_pt = "Masmorras e Dragões"` (nome oficial PT-BR quando existe); cenário `Forgotten Realms` → `name_pt = "Reinos Esquecidos"`. Migration: `ALTER TABLE systems ADD COLUMN IF NOT EXISTS name_pt TEXT; ALTER TABLE scenarios ADD COLUMN IF NOT EXISTS name_pt TEXT;`. Backend: atualizar endpoints `GET /api/v1/systems`, `GET /api/v1/scenarios` para retornar `name_pt`. CRUD admin de sistemas/cenários deve incluir campo `name_pt` no payload de criação/edição. | `database/migration_17_name_pt.sql`, `backend/src/routes/systems.ts`, `backend/src/routes/scenarios.ts` | pendente | **Score GUT: 80 (4×5×4).** Campo opcional — maioria dos sistemas não tem nome PT oficial. Não afeta mesas existentes. Permite que o mestre veja e escolha qual versão usar. |
