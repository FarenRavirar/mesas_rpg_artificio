...
> Fluxo completo de importação inteligente de JSON do Discord — Parser Python enriquecido, normalização backend, auto-preenchimento de formulário, abertura automática de blocos, persistência com overrides e página pública indistinguível de mesa manual.

| ID | Fase | Tipo | GUT | Titulo | Descrição objetiva | Arquivos esperados | Status | Observação |
|---|---|---|---|---|---|---|---|---|
| 137 | Fase 4 | fullstack | 5/5/5 | **[BLOQUEADOR] Diagnosticar e corrigir erro 500 em POST /api/v1/gm/tables** | Investigar causa raiz do erro 500 ao criar mesa. Hipóteses: (1) Frontend envia campos que rota não sanitiza; (2) Mismatch entre nomes de campo (banner_url vs cover_url); (3) Arrays ou JSON em formato incompatível; (4) Coluna ausente no banco; (5) Tipo incompatível. Capturar payload real enviado, adicionar logs estruturados antes de sanitização e persistência, envolver bloco crítico com tratamento de erro que exponha causa real. Validar compatibilidade entre tipos TypeScript e schema do banco. Confirmar que todas as colunas existem. Testar com payload mínimo e expandir incrementalmente. **SOLUÇÃO APLICADA (05/04/2026):** Causa raiz confirmada - constraint `price_value_required` violada quando `price_type='paga'` mas `price_value` é null/undefined/0. Adicionada validação explícita antes da transação que retorna erro 400 claro. Constraint removida do banco - validação agora é feita no código. Build validado sem erros TypeScript. | backend/src/routes/gmPanel.ts (linhas 383-390, 445), ERRORS_SOLUTIONS.md (E128) | concluido | **Prioridade 0 - Bloqueador crítico resolvido.** Fluxo de criação/importação de mesas desbloqueado. Ver E128 para detalhes completos da solução. |
| 138 | Fase 4 | fullstack | 5/5/4 | **[CRÍTICO] Corrigir banner não preenchido no fluxo de importação** | Garantir que banner do anúncio importado: (1) seja extraído do JSON (attachments); (2) chegue ao candidato; (3) preencha o formulário de revisão; (4) apareça no preview; (5) seja persistido no banco; (6) apareça na página pública. Verificar ponta a ponta: parser Python extrai banner_url → normalizeExporterPayload preserva → parseExporterMessage mantém → candidateToFormData mapeia → formulário mostra preview → POST /api/v1/gm/tables aceita → candidateService persiste → API pública retorna → MesaPage renderiza. **SOLUÇÃO APLICADA (05/04/2026):** Causa raiz confirmada - bug de mapeamento em candidateToFormData.ts linha 276. Função verificava `parsedContent.banner_url` ao invés de `enrichedJson.banner_url`. Corrigido para usar `enrichedJson` (merge completo). Mesmo bug corrigido para `avatar_url` linha 287. Build validado sem erros TypeScript. | frontend/src/utils/candidateToFormData.ts (linhas 276, 287), ERRORS_SOLUTIONS.md (E129) | concluido | **Prioridade 1 - Quebra funcional resolvida.** Banner e avatar agora são corretamente mapeados do parser Python para o formulário. Ver E129 para detalhes completos da solução. |
| 139 | Fase 4 | fullstack | 5/5/4 | **[CRÍTICO] Corrigir descrição incompleta - definir regra editorial** | Definir e implementar regra de montagem da descrição longa da mesa. Sistema atual trata synopsis como descrição completa, mas anúncios reais têm: título, sinopse, regras, observações, diferenciais, instruções de inscrição, benefícios. Separar claramente: (1) synopsis (sinopse curta); (2) description (descrição principal longa); (3) rules/observations (regras e observações); (4) signupText (instruções de inscrição - NÃO misturar com description). Parser deve extrair blocos separados. Normalização deve preservar. FormPatch deve mapear corretamente. Persistência deve salvar descrição completa sem truncamento. Página pública deve renderizar texto completo. | backend/src/services/aggregator/parser/discord_message_parser.py, backend/src/domain/aggregator/normalizeExporterPayload.ts, backend/src/domain/aggregator/parseExporterMessage.ts, frontend/src/utils/candidateToFormData.ts, backend/src/routes/gmPanel.ts, frontend/src/pages/MesaPage.tsx | pendente | **Prioridade 2 - Qualidade editorial.** Decisão funcional obrigatória: synopsis ≠ description final. Ver plano_json 2.md linhas 357-430 para backlog detalhado por arquivo. |
| 127 | Fase 4 | backend | 5/5/5 | Parser Python - Campos de importação inteligente | Expandir parser Python com 11 novos campos: banner_url (primeiro attachment de imagem), avatar_url (author.avatarUrl), external_links (array de URLs de formulários/sites), is_paid (boolean), priceText (string), signupText (texto de inscrição), requires_pc (boolean), requires_camera (boolean), requires_microphone (boolean), is_ongoing (boolean), reviewFlags (array de avisos). Atualizar schemas Pydantic. Implementar extract_requires_pc() que ainda está ausente. | backend/src/services/aggregator/parser/discord_message_parser.py, backend/src/services/aggregator/parser/schemas.py | pendente | Campos já extraídos: setting_name, setting_styles (REQ-28 Fase 1). Novos campos completam o conjunto necessário para auto-preenchimento. Bloqueado por: item 139 (descrição incompleta). Critério: npm run build passa + parser retorna 11 novos campos + schemas validam corretamente |
| 128 | Fase 4 | backend | 5/5/5 | Atualizar pythonParserService com novos campos | Atualizar interface ParsedMessageResult em pythonParserService.ts para incluir os 11 novos campos do parser Python. Adicionar logs de comando executado, sucesso, erro e tempo de execução. Fallback claro quando Python falhar. | backend/src/services/aggregator/pythonParserService.ts | pendente | Não engolir erro silenciosamente. Logar quando Python rodou e quando caiu no fallback. Bloqueado por: item 127. Critério: npm run build passa + interface atualizada + logs implementados |
| 129 | Fase 4 | backend | 5/5/5 | Normalização - Priorizar enrichedFields | Atualizar normalizeExporterPayload.ts para priorizar enrichedFields sobre fallback TypeScript. Merge correto: { ...rawPayload, ...enrichedFields } (enrichedFields por último). Preservar attachments, embeds, mentions, author e todos os campos enriquecidos. Logar quais campos vieram do parser e quais ficaram vazios. Nunca sobrescrever dados validados do parser com parsing fraco. | backend/src/domain/aggregator/normalizeExporterPayload.ts | pendente | Regra crítica: não há perda silenciosa de campos. parsed_json final contém tudo que o parser devolveu. Ver erro E122. Bloqueado por: item 139 (descrição incompleta). Critério: npm run build passa + enrichedFields priorizados + logs implementados |
| 130 | Fase 4 | backend | 4/5/4 | parseExporterMessage - Priorizar enrichedFields | Atualizar parseExporterMessage.ts para priorizar enrichedFields em todos os campos que existirem. Manter fallback regex apenas quando campo estiver ausente. Não sobrescrever setting_name e setting_styles do parser com parsing fraco. Separar masterText e recruiterName. Determinar is_paid, billing_text, is_ongoing, candidate_kind, recruitment_channels preliminares, requirements preliminares. Regra: priorizar enrichedFields; fallback para parsing secundário só quando necessário. | backend/src/domain/aggregator/parseExporterMessage.ts | pendente | O TypeScript não deve mais ignorar o Python. Candidato sai com dados suficientes para revisão. Bloqueado por: item 139 (descrição incompleta). Critério: npm run build passa + enrichedFields priorizados + fallback funcional |
| 131 | Fase 4 | frontend | 5/5/5 | candidateToFormData - Auto-preenchimento completo | Expandir candidateToFormData.ts para mapear todos os campos do parser: sistema pré-selecionado via system_path_slug (busca fuzzy na árvore), cobrança marcada quando is_paid=true, valor preenchido via priceText, canal Discord criado automaticamente quando origem for Discord e authorUsername/authorHandle presentes, formulário/WhatsApp adicionados quando detectados em external_links/signupText, banner importado via banner_url, avatar do autor via avatar_url (apenas preview, não persiste), requisitos técnicos marcados (requires_pc/camera/microphone), mesa em andamento marcada (is_ongoing), cenário e estilos preenchidos (setting_name, setting_styles), agenda aproveitada (sessions[] ou texto bruto). Publisher_role forçado como 'announcer' para candidatos importados. | frontend/src/utils/candidateToFormData.ts | pendente | Sistema não exige redigitação do que já estava no JSON. Ver erros E123, E124. Bloqueado por: item 139 (descrição incompleta). Critério: npm run build passa + todos os campos mapeados + auto-preenchimento funcional |
| 132 | Fase 4 | frontend | 4/5/4 | Comportamento inteligente da UI no formulário de revisão | Atualizar formulário de revisão para comportamento inteligente: (1) Preencher campos automaticamente via candidateToFormData; (2) Mostrar preview de banner quando banner_url existir (com onError handler); (3) Mostrar preview circular de avatar do autor quando disponível (apenas visual, não persiste); (4) Marcar cobrança e valor quando is_paid=true; (5) Popular canais de recrutamento automaticamente (Discord, formulário, WhatsApp); (6) Marcar requisitos técnicos (PC, câmera, microfone); (7) Marcar mesa em andamento; (8) Preencher cenário e estilos; (9) Usar repetidor de agenda se sessions[] existir, ou fallback para texto bruto. Admin pode editar qualquer campo antes de aprovar. IMPORTANTE: Não assumir blocos colapsáveis se não existirem no código atual - documentar comportamento real necessário. | frontend/src/pages/GestaoPage.tsx, frontend/src/pages/PainelMestrePage.tsx | pendente | Sistema não exige redigitação do que já estava no JSON. Bloqueado por: item 131. Critério: npm run build passa + UI inteligente funcional + preview de imagens funcional |
| 133 | Fase 4 | backend | 5/5/5 | Endpoint accept com overrides opcionais | Modificar endpoint PATCH /api/v1/aggregator/candidates/:id/accept para aceitar body opcional com overrides. Validar apenas campos conhecidos (whitelist). Sanitizar arrays e strings. Passar overrides para candidateService.acceptCandidate(candidateId, overrides). Merge de overrides com parsed_json no service: const finalData = { ...candidate.parsed_json, ...overrides }. Aprovação não deve mais depender apenas de body vazio - aceitar payload revisado opcional. | backend/src/routes/aggregatorReview.ts, backend/src/services/aggregator/candidateService.ts | pendente | Backend persiste dados revisados (não apenas brutos). Ver erros E125, E126. Bloqueado por: item 139 (descrição incompleta). Critério: npm run build passa + endpoint aceita overrides + validação funcional |
| 134 | Fase 4 | backend | 5/5/5 | candidateService - Persistir todos os campos revisados | Atualizar candidateService.acceptCandidate() para ler setting_name, setting_styles, banner_url (ou cover_url - alinhar nomenclatura), billing_text, publisher_role, master_display_name, canais de recrutamento, requisitos técnicos (requires_pc, requires_camera, requires_microphone), status de andamento (is_ongoing), descrição completa (não truncada) de parsed_json ou overrides. Adicionar campos ao INSERT de tables. Validar persistência com query direta no banco após aprovação. Garantir que fluxo manual e fluxo importado convergem para o mesmo contrato de mesa. | backend/src/services/aggregator/candidateService.ts | pendente | Regra: cenário/estilos/banner/cobrança/recrutamento/requisitos/descrição completa chegam até a tabela tables final. Mesa importada publicada fica indistinguível de mesa manual. Bloqueado por: item 133. Critério: npm run build passa + todos os campos persistidos + query de validação passa |
| 135 | Fase 4 | frontend | 4/4/4 | MesaPage - Renderizar campos importados | Atualizar MesaPage.tsx para renderizar todos os campos importados: cenário e estilos, banner, requisitos técnicos (PC, câmera, microfone), status de andamento, canais de recrutamento, descrição completa (não resumida indevidamente), cobrança detalhada. Garantir que mesa importada seja visualmente indistinguível de mesa criada manualmente. | frontend/src/pages/MesaPage.tsx | pendente | Validação final: mesa importada aprovada deve ter mesma qualidade visual de mesa manual. Bloqueado por: item 134. Critério: npm run build passa + todos os campos renderizados + mesa importada indistinguível de manual |
| 136 | Fase 4 | fullstack | 5/5/5 | Testes E2E do fluxo completo | Criar teste E2E que valida fluxo completo: (1) Importar JSON com todos os campos (descrição longa, banner, cobrança, requisitos); (2) Parser extrai dados; (3) Normalização preserva campos; (4) Formulário de revisão pré-preenchido; (5) Comportamento inteligente da UI ativo; (6) Admin edita campo; (7) Aprovação persiste override; (8) Mesa publicada exibe todos os dados sem perda. Usar anúncio grande do JSON real que tenha sinopse, observações, benefícios e instruções de inscrição. Comparar texto cru da mensagem vs descrição final renderizada. | backend/tests/e2e/importacao-inteligente.test.ts | pendente | Teste crítico para garantir que não há perda de dados em nenhuma camada. Bloqueado por: item 139 (descrição incompleta) e itens 127-135. Critério: npm run build passa + teste E2E passa + fluxo completo validado |
