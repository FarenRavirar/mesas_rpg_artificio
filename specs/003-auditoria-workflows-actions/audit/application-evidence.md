...
### T034: Valida��o de Rollback At�mico (Off-Happy-Path) em Beta ?

**Status:** APPLIED & VALIDATED
**Priority:** CRITICAL
**Date:** 2026-04-23

**Evid�ncia de Valida��o:**
- **Run URL:** https://github.com/FarenRavirar/mesas_rpg_artificio/actions/runs/24863641138
- **M�todo de Falha Induzida:** Um script observador remoto aguardou a finaliza��o do job deploy-app e a recria��o do container mesas-beta-frontend. Imediatamente, substituiu as configura��es do Nginx injetando uma diretiva eturn 500; na rota cr�tica /api/v1/tables.
- **Comportamento do Pipeline:**
  ...
  4. O snapshot at�mico pg_restore foi restaurado com sucesso.
  5. docker compose up -d --force-recreate recriou os containers for�ando o uso da imagem/configura��o sem os artefatos quebrados.
...
smoke	Smoke Tests	2026-04-23T23:44:12.1998877Z out: Rollback completo conclu�do com sucesso. Workflow ser� marcado como falha para alerta operacional.
``

**Estado dos Containers Ap�s o Rollback:**
``bash
...
**Conclus�o da T034:** O teste provou categoricamente que falhas estruturais cr�ticas no runtime/deploy ativam o freio de seguran�a (E150), restaurando a integridade da aplica��o e sinalizando o CI/CD adequadamente, fechando assim a Phase 5 com sucesso e sem res�duos.
