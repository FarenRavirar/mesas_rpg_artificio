...
| E087 | `Cannot GET /auth/google/callback` após login Google no beta | Desalinhamento de rota OAuth entre callback configurado no Google/compose (`/auth/google/callback`) e rotas expostas pela API (`/api/v1/auth/...`), combinado com proxy Nginx genérico em `/auth/` que também capturava indevidamente a rota SPA `/auth/callback` do frontend | Abrir URL de callback no navegador e observar 404; inspecionar `server.ts` e `nginx.conf` para confirmar ausência de alias `/auth` na API e conflito de proxy com rota frontend de callback | Expor alias de compatibilidade no backend (`app.use('/auth', authRoutes)`) e restringir Nginx para proxy apenas de `/auth/google` e `/auth/google/callback`, deixando `/auth/callback` cair no fallback SPA | Manter `GOOGLE_CALLBACK_URL`, rotas do backend e regras do Nginx alinhadas. Evitar `location /auth/` genérica quando o frontend usa rotas SPA com o mesmo prefixo |
