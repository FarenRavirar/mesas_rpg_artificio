...
| E092 | `ENOENT: no such file or directory, open '/arvores_de_sistemas.md'` ao executar `systemsTreeImport.js` no container | O arquivo `arvores_de_sistemas.md` está na raiz do repositório mas o contexto de build do Docker é `./backend`, tornando-o inacessível durante o build | `docker exec mesas-beta-api find /app -name 'arvores_de_sistemas.md'` retorna vazio | Alterar contexto de build no `docker-compose.beta.yml` para `.` (raiz) e ajustar paths no `backend/Dockerfile` para refletir o novo contexto; copiar `arvores_de_sistemas.md` explicitamente no estágio builder | Sempre que um script do backend precisar de arquivo da raiz do repositório, garantir que o contexto de build do Docker inclua esse arquivo |
