...
exports.DEV_FEEDBACK_LIMITS = void 0;
exports.parseDevFeedbackInput = parseDevFeedbackInput;
/**
...
function readTruncatedOrNull(value, max) {
...
function normalizeConsoleErrors(value) {
    ...
    for (const item of value) {
        if (out.length >= exports.DEV_FEEDBACK_LIMITS.arrayCap)
            break;
        ...
        out.push({
            ...
            message: trunc(message, exports.DEV_FEEDBACK_LIMITS.message),
            ts: ts.length > 0 ? trunc(ts, 40) : null,
        });
    }
    return out;
}
function normalizeNetworkErrors(value) {
    if (!Array.isArray(value))
        return [];
    const out = [];
    for (const item of value) {
        ...
        const ts = readString(row.ts).trim();
        out.push({
            url: trunc(url, exports.DEV_FEEDBACK_LIMITS.url),
...
function normalizeScreenshot(value) {
    if (typeof value !== 'string')
        return null;
    const s = value.trim();
    if (!SCREENSHOT_RE.test(s))
        return null;
    if (s.length > exports.DEV_FEEDBACK_LIMITS.screenshotChars)
        return null;
    return s;
}
function parseDevFeedbackInput(raw) {
    ...
    if (emailRaw.length > 0) {
        if (emailRaw.length > exports.DEV_FEEDBACK_LIMITS.email || !EMAIL_RE.test(emailRaw)) {
    ...
    return {
        ...
        value: {
            kind: kindRaw,
            title: trunc(title, exports.DEV_FEEDBACK_LIMITS.title),
            description: trunc(description, exports.DEV_FEEDBACK_LIMITS.description),
            contact_email: contactEmail,
            page_url: readTruncatedOrNull(body.page_url, exports.DEV_FEEDBACK_LIMITS.url),
            route_path: readTruncatedOrNull(body.route_path, exports.DEV_FEEDBACK_LIMITS.routePath),
            page_title: readTruncatedOrNull(body.page_title, exports.DEV_FEEDBACK_LIMITS.pageTitle),
            environment: readTruncatedOrNull(body.environment, exports.DEV_FEEDBACK_LIMITS.environment),
            user_agent: readTruncatedOrNull(body.user_agent, exports.DEV_FEEDBACK_LIMITS.userAgent),
            viewport: readTruncatedOrNull(body.viewport, exports.DEV_FEEDBACK_LIMITS.viewport),
