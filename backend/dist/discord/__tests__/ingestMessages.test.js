...
const ingestMessages_1 = require("../ingestMessages");
describe('listForumThreads', () => {
    ...
    it('paginates public archived forum threads while Discord reports has_more', async () => {
        ...
        const threads = await (0, ingestMessages_1.listForumThreads)({
            guildId: 'guild-1',
            forumChannelId: 'forum-1',
            token: 'bot-token',
        });
        expect(threads.map((thread) => thread.id).sort()).toEqual(['active-1', 'archived-1', 'archived-2']);
        expect(fetchMock).toHaveBeenNthCalledWith(3, 'https://discord.com/api/v10/channels/forum-1/threads/archived/public?limit=100&before=2026-05-10T00%3A00%3A00.000Z', expect.any(Object));
    });
});
