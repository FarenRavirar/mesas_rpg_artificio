...
const draftValidation_1 = require("../draftValidation");
describe('assertDraftReadyTransition (T-F1-02)', () => {
    it('allows any patch when status is not changing to ready', () => {
        expect((0, draftValidation_1.assertDraftReadyTransition)({
    ...
    it('rejects ready when patch payload still has missing fields', () => {
        const result = (0, draftValidation_1.assertDraftReadyTransition)({
            patchStatus: 'ready',
    ...
    it('treats null patch payload as absence and uses current draft', () => {
        const blocked = (0, draftValidation_1.assertDraftReadyTransition)({
            patchStatus: 'ready',
            patchPayloadMissing: null,
            currentPayloadMissing: ['contact_url'],
        });
        expect(blocked.allowed).toBe(false);
        const allowed = (0, draftValidation_1.assertDraftReadyTransition)({
            patchStatus: 'ready',
            patchPayloadMissing: null,
            currentPayloadMissing: [],
        });
