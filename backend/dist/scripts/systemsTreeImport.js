...
const normalizeLineLabel = (raw) => raw.replace(/\s+/g, ' ').trim();
const extractMarkdownNodes = (markdown) => {
...
const buildAliases = (name, rootSegment, metadata) => {
...
const resolveSourceFilePath = () => {
    return path_1.default.resolve(__dirname, '../../arvores_de_sistemas.md');
};
const run = async () => {
    const sourcePath = resolveSourceFilePath();
    const markdown = fs_1.default.readFileSync(sourcePath, 'utf8');
    ...
    await db_1.db.transaction().execute(async (trx) => {
        ...
        for (const row of existingSystems) {
            if (row.path_slug)
                existingMap.set(row.path_slug, row.id);
        }
        const stack = [];
        for (const node of parsedNodes) {
            ...
            const [upserted] = await trx
                ...
                .onConflict((oc) => oc.column('path_slug').doUpdateSet({
                name: label,
                slug: globalSlug,
                description,
                parent_id: parent?.systemId ?? null,
