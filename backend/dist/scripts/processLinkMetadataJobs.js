...
exports.processPendingLinks = processPendingLinks;
...
async function fetchOgMetadata(url) {
    ...
    try {
        ...
        return {
            title: titleMatch ? (0, he_1.decode)(titleMatch[1]) : null,
            description: descMatch ? (0, he_1.decode)(descMatch[1]) : null,
...
function getNextRetryInterval(failCount) {
...
async function processPendingLinks() {
    ...
    try {
        ...
        const res = await (0, kysely_1.sql) `
    ...
    `.execute(db_1.db);
        ...
        for (const job of jobs) {
            try {
                ...
                await db_1.db.updateTable('user_links')
                    .set({
                    title: metadata.title,
                    description: metadata.description,
                    thumbnail_url: metadata.thumbnail_url,
                    metadata_status: 'success',
                    metadata_fetched_at: (0, kysely_1.sql) `NOW()`,
                ...
                await db_1.db.updateTable('user_links')
                    ...
                    metadata_next_retry_at: interval ? (0, kysely_1.sql) `NOW() + ${kysely_1.sql.raw(`INTERVAL '${interval}'`)}` : null,
                    updated_at: (0, kysely_1.sql) `NOW()`
                })
                    .where('id', '=', job.id)
                    .execute();
