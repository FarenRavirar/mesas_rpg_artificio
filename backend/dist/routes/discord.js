 ...
 */
router.get('/discord/connect', auth_1.authMiddleware, async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        return res.status(401).json({ error: 'Não autenticado' });
    }
    // CORREÇÃO P02, P06, P20: Validar se usuário tem perfil GM antes de iniciar OAuth
    const gmProfile = await index_1.db
...
router.get('/discord/callback', async (req, res) => {
    ...
    try {
        ...
        const isInCovil = guilds.some((guild) => guild.id === process.env.DISCORD_GUILD_ID);
 ...
 */
router.delete('/discord/disconnect', auth_1.authMiddleware, async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        return res.status(401).json({ error: 'Não autenticado' });
    }
 ...
 */
router.post('/discord/verify-covil', auth_1.authMiddleware, async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        return res.status(401).json({ error: 'Não autenticado' });
    }
