...
const CONTACT_CHANNELS = ['whatsapp', 'discord', 'phone', 'email', 'facebook', 'instagram', 'form'];
...
const sanitizeOptionalTier = (value) => {
...
const sanitizePublisherRole = (value) => {
    if (typeof value !== 'string')
        return 'gm';
    const normalized = value.trim().toLowerCase();
    return normalized === 'announcer' ? 'announcer' : 'gm';
};
const sanitizeContactsPayload = (value) => {
    ...
    for (let index = 0; index < value.length; index += 1) {
        ...
        if (!CONTACT_CHANNELS.includes(rawChannel)) {
            return { contacts: [], error: `Canal inválido no contato #${index + 1}.` };
        }
        const safeChannel = rawChannel;
...
const isDdalEligibleSystem = async (systemId) => {
    const system = await db_1.db
        .selectFrom('systems')
        .select(['path_slug'])
        .where('id', '=', systemId)
        .executeTakeFirst();
...
router.post('/tables', auth_1.authMiddleware, async (req, res) => {
    ...
    synopsis_narrative, benefits_text, table_gm_bio, } = req.body;
    ...
    let vttPlatformUuid = null;
    if (safeVttPlatformId && safeVttPlatformId !== 'custom') {
        try {
            const vttPlatform = await db_1.db
    ...
    try {
        ...
        const newTable = await db_1.db.transaction().execute(async (trx) => {
            ...
            await trx
                ...
                .values(contactsPayload.contacts.map((contact) => ({
            ...
            if (schedules && Array.isArray(schedules) && schedules.length > 0) {
                ...
                await trx
                    ...
                    .values(validSchedules.map((schedule, index) => ({
                    table_id: insertedTable.id,
...
router.put('/tables/:id', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (hasOwn('slots_total') || hasOwn('slots_open')) {
            ...
            const finalSlotsTotal = parsedSlotsTotal ?? (await db_1.db.selectFrom('tables').select('slots_total').where('id', '=', id).executeTakeFirst())?.slots_total ?? 4;
            const finalSlotsOpen = parsedSlotsOpen ?? (await db_1.db.selectFrom('tables').select('slots_open').where('id', '=', id).executeTakeFirst())?.slots_open ?? finalSlotsTotal;
        ...
        const updated = await db_1.db.transaction().execute(async (trx) => {
...
router.put('/admin/tables/:id', auth_1.authMiddleware, async (req, res) => {
    const userRole = req.user.role;
    if (userRole !== 'admin') {
        return res.status(403).json({ error: 'Acesso negado. Apenas administradores.' });
    }
    const { id } = req.params;
    const { title, description, system_id, scenario_id, type, audience, age_rating, modality, game_platform, communication_platform, price_type, price_value, price_frequency, slots_total, slots_filled, language, experience_level, table_level, starts_at, city, state, content_warnings, safety_tools, status, frequency, frequency_custom, rules_notes, banner_url, 
    // CORREÇÃO B06: Campos REQ-26/28 estavam ausentes no admin PUT — zerados silenciosamente
    master_display_name, campaign_length, level_range, billing_text, session_zero_free, synopsis, style_text, listing_excerpt, technical_requirements, requires_pc, requires_camera, requires_microphone, setting_name, setting_styles, synopsis_narrative, benefits_text, table_gm_bio, 
    // REQ-21: Novos campos
    custom_scenario, style_tags, 
    ...
    try {
        ...
        if (scenario_id) {
            const scenarioExists = await db_1.db
        ...
        const updated = await db_1.db
            ...
            // REQ-05: Flag Covil do Lich (admin only)
            // CORREÇÃO B03: Validar role admin para is_covil
            is_covil: hasOwn('is_covil') ? (req.user?.role === 'admin' && parseOptionalBoolean(is_covil) ? true : false) : undefined,
        })
            .where('id', '=', id)
            .returning([
            'id',
...
router.delete('/admin/tables/:id', auth_1.authMiddleware, async (req, res) => {
    const userRole = req.user.role;
    if (userRole !== 'admin') {
        return res.status(403).json({ error: 'Acesso negado. Apenas administradores.' });
    }
