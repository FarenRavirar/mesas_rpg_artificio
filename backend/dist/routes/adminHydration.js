...
router.post('/sync/hydrate', auth_1.authMiddleware, async (req, res) => {
    console.log('[HYDRATE-DEBUG] req.user:', JSON.stringify(req.user));
    ...
    const logs = [];
    try {
        await prod_1.prodDb.selectFrom('users').select('id').limit(1).execute();
        ...
        await db_1.db.transaction().execute(async (trx) => {
            ...
            for (const tableName of tablesToSync) {
                ...
                if (tableName === 'tables') {
                    ...
                    prodRecords = await prod_1.prodDb
                        .selectFrom('tables')
                    ...
                    prodRecords = await prod_1.prodDb.selectFrom(tableName).selectAll().execute();
                }
                // T010: Contadores
                ...
                for (const record of prodRecords) {
                    try {
                        ...
                        const safeRecord = Object.fromEntries(Object.entries(rawSafeRecord).filter(([key]) => allowedFields.includes(key)));
                        ...
                        switch (tableName) {
                            ...
                            case 'communication_platforms':
                                ...
                                result = await trx.insertInto(tableName)
                                    .values(safeRecord)
                                    .onConflict((oc) => oc.column('slug').doUpdateSet(updateObj))
                                    .returning(['id', (0, kysely_1.sql) `xmax`.as('xmax')])
                                    .executeTakeFirst();
                                break;
                            case 'sources':
                            ...
                            case 'tables':
                                ...
                                const resolvedRecord = { ...safeRecord };
                                ...
                                const updateObjTables = { ...resolvedRecord };
                                delete updateObjTables.id;
                                delete updateObjTables.created_at;
                                result = await trx.insertInto('tables')
                                    .values(resolvedRecord)
                                    .onConflict((oc) => oc.column('id').doUpdateSet(updateObjTables))
                                    .returning(['id', (0, kysely_1.sql) `xmax`.as('xmax')])
                                    .executeTakeFirst();
                                break;
                            case 'table_contacts':
                            ...
                            case 'table_platforms':
                                ...
                                if (Object.keys(updateObj).length > 0) {
                                    result = await trx.insertInto(tableName)
                                        ...
                                        .returning(['table_id', (0, kysely_1.sql) `xmax`.as('xmax')])
                                        .executeTakeFirst();
                                }
                                else {
                                    result = await trx.insertInto(tableName)
                            ...
                            case 'player_profiles':
                                result = await trx.insertInto(tableName)
                                    ...
                                    .returning(['user_id', (0, kysely_1.sql) `xmax`.as('xmax')])
                                    .executeTakeFirst();
                                break;
                            case 'auth_providers':
                                result = await trx.insertInto(tableName)
