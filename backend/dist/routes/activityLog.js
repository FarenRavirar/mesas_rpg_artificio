...
router.use(rateLimit_1.authRateLimiter);
...
const parseActionFilters = (rawAction) => {
...
router.get('/activity', async (req, res) => {
    ...
    try {
        ...
        let query = db_1.db
            ...
            'al.actor_id',
            'al.target_user_id',
            (0, kysely_1.sql) `COALESCE(actor_p.display_name, actor_u.username)`.as('actor_name'),
            'actor_p.avatar_url as actor_avatar_url',
            (0, kysely_1.sql) `COALESCE(target_p.display_name, target_u.username)`.as('target_name'),
        ...
        const [actors, targetUsers, availableActionsRows] = await Promise.all([
            db_1.db
                ...
                .select([
                'u.id as id',
                (0, kysely_1.sql) `COALESCE(p.display_name, u.username)`.as('name'),
