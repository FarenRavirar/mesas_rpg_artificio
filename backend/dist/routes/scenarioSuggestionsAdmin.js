...
router.get('/scenario-suggestions', async (req, res) => {
    try {
        ...
        let query = db_1.db.selectFrom('scenario_suggestions').selectAll().orderBy('created_at', 'desc');
...
router.patch('/scenario-suggestions/:id/approve', async (req, res) => {
    try {
        const { id } = req.params;
...
router.patch('/scenario-suggestions/:id/reject', async (req, res) => {
    try {
        const { id } = req.params;
