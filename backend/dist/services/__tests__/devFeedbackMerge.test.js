...
const devFeedbackMerge_1 = require("../devFeedbackMerge");
const base = (over) => ({
    id: over.id ?? 'id',
    kind: over.kind ?? 'bug',
...
describe('buildMerge', () => {
    it('une console/network errors com dedup', () => {
        ...
        const out = (0, devFeedbackMerge_1.buildMerge)(primary, [src]);
    ...
    it('cria snapshot completo de cada secundario em merged_sources', () => {
        ...
        const snap = out.merged_sources[0];
    ...
    it('preserva merged_sources ja existentes no destino (append)', () => {
        ...
        const out = (0, devFeedbackMerge_1.buildMerge)(primary, [base({ id: 'new' })]);
        expect(out.merged_sources).toHaveLength(2);
        expect(out.merged_sources[0].id).toBe('old');
        expect(out.merged_sources[1].id).toBe('new');
    });
    it('limita o total de erros ao cap', () => {
        const many = Array.from({ length: devFeedbackMerge_1.MAX_MERGED_ERRORS + 30 }, (_, i) => ({ level: 'error', message: `e${i}` }));
        const primary = base({ id: 'p', console_errors: many });
        const out = (0, devFeedbackMerge_1.buildMerge)(primary, [base({ id: 's', console_errors: [{ level: 'error', message: 'extra' }] })]);
        expect(out.console_errors.length).toBe(devFeedbackMerge_1.MAX_MERGED_ERRORS);
    });
    it('agrega multiplos secundarios', () => {
        const primary = base({ id: 'p' });
        const out = (0, devFeedbackMerge_1.buildMerge)(primary, [base({ id: 's1' }), base({ id: 's2' }), base({ id: 's3' })]);
