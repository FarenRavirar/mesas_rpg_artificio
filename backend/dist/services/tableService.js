...
class TableService {
     ...
     */
    static async isDdalEligibleSystem(systemId) {
     ...
     */
    static async validateVttPlatform(vttPlatformId) {
     ...
     */
    static async validateCommunicationPlatform(communicationPlatformId, communicationPlatformLegacy) {
        ...
        const platform = await db_1.db
            ...
            .where((eb) => eb.or([
            eb('slug', '=', legacySlug),
            eb((0, kysely_1.sql) `LOWER(name)`, '=', legacy.toLowerCase()),
        ]))
            .executeTakeFirst();
        if (platform) {
     ...
     */
    static prepareTableData(data, gmProfileId, vttPlatformUuid, communicationPlatformUuid, communicationPlatformLegacy, slug, userRole) {
