...
exports.AdminDevToolsPage = AdminDevToolsPage;
...
const createInitialQuickResults = () => {
    const results = {};
...
const parseDiscordChannelLink = (value) => {
...
const getPayloadMessageCount = (payload) => {
    if (!payload || typeof payload !== 'object')
        return 0;
    const messages = payload.messages;
    return Array.isArray(messages) ? messages.length : 0;
};
const splitPayloadForImport = (payload, chunkSize) => {
    if (!payload || typeof payload !== 'object')
        return [payload];
    const root = payload;
...
const sanitizeDiscordExporterJson = (rawJson) => {
    ...
    try {
        ...
        try {
            const repaired = (0, jsonrepair_1.jsonrepair)(rawJson);
...
const aggregateImportSummaries = (summaries, dryRun) => {
...
const summarizePayload = (payload) => {
    ...
    if (messageCount !== null)
        lines.push(`Mensagens encontradas: ${messageCount}`);
    if (guildName)
        lines.push(`Servidor detectado: ${guildName}`);
    if (channelName)
        lines.push(`Canal detectado: ${channelName}`);
    ...
    if (lines.length === 0)
        lines.push('Arquivo carregado. Estrutura fora do padrão esperado do exporter, mas pode ser testada no import.');
    return lines;
};
function AdminDevToolsPage() {
    const { user, token: sessionToken } = (0, AuthContext_1.useAuth)();
    ...
    const [quickResults, setQuickResults] = (0, react_1.useState)(createInitialQuickResults);
    const [runningQuickTests, setRunningQuickTests] = (0, react_1.useState)(false);
    const [channelUrl, setChannelUrl] = (0, react_1.useState)('');
    const [sourceName, setSourceName] = (0, react_1.useState)('');
    const [sourceCreateFeedback, setSourceCreateFeedback] = (0, react_1.useState)(null);
    const [sourceCreateError, setSourceCreateError] = (0, react_1.useState)(null);
    const [creatingSource, setCreatingSource] = (0, react_1.useState)(false);
    const [importSourceId, setImportSourceId] = (0, react_1.useState)('');
    const [importDryRun, setImportDryRun] = (0, react_1.useState)(true);
    const [fileName, setFileName] = (0, react_1.useState)('');
    const [payloadPreview, setPayloadPreview] = (0, react_1.useState)([]);
    const [parsedPayload, setParsedPayload] = (0, react_1.useState)(null);
    const [dragActive, setDragActive] = (0, react_1.useState)(false);
    const [importFeedback, setImportFeedback] = (0, react_1.useState)(null);
    const [importError, setImportError] = (0, react_1.useState)(null);
    const [importResult, setImportResult] = (0, react_1.useState)(null);
    const [importingJson, setImportingJson] = (0, react_1.useState)(false);
    const [jsonWasRepaired, setJsonWasRepaired] = (0, react_1.useState)(false);
    const [jsonRepairFailed, setJsonRepairFailed] = (0, react_1.useState)(false);
    const [jsonRepairBannerDismissed, setJsonRepairBannerDismissed] = (0, react_1.useState)(false);
    const [importProgress, setImportProgress] = (0, react_1.useState)(null);
    const autoRunTokenRef = (0, react_1.useRef)('');
    (0, react_1.useEffect)(() => {
    ...
    const parsedChannelLink = (0, react_1.useMemo)(() => parseDiscordChannelLink(channelUrl), [channelUrl]);
    ...
    const executeRouteTest = async (route) => {
    ...
    const handleImportFileInput = async (event) => {
        const file = event.target.files?.[0];
        if (!file)
            return;
        await loadJsonFile(file);
    };
    const importChunkInfo = (0, react_1.useMemo)(() => {
    ...
    const getQuickBadgeStyle = (state) => {
        if (state === 'success') {
            return {
                color: '#22c55e',
      ...
      <div style={{ maxWidth: 980, margin: '0 auto' }}>
        ...
        <section style={{
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
            <input id="admin-jwt-token-input" type={revealed ? 'text' : 'password'} value={token} onChange={(e) => saveToken(e.target.value)} onFocus={() => setRevealed(true)} onBlur={() => setRevealed(false)} placeholder="Cole aqui o JWT admin" style={{
            flex: 1,
            minWidth: 300,
            ...
            <button id="admin-clear-token-btn" onClick={() => saveToken('')} style={{
            background: 'rgba(239,68,68,0.15)',
        ...
        <section style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.8rem', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button id="admin-run-all-tests-btn" onClick={runAllQuickTests} disabled={runningQuickTests} style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '0.65rem', marginTop: '0.85rem' }}>
            ...
            return (<article key={test.id} style={{
                    borderRadius: 10,
                    border: badge.border,
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.9rem', flexWrap: 'wrap' }}>
            <input id="admin-custom-path-input" type="text" value={customPath} onChange={(e) => setCustomPath(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && runCustom()} placeholder="Teste customizado: /api/v1/..." style={{
            flex: 1,
            minWidth: 260,
            background: 'rgba(255,255,255,0.04)',
            border: '1px solid rgba(255,255,255,0.1)',
            ...
            <button id="admin-run-custom-btn" onClick={runCustom} disabled={loading} style={{
        ...
        <section style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '0.5rem' }}>
            <input id="admin-source-channel-link-input" type="text" value={channelUrl} onChange={(e) => setChannelUrl(e.target.value)} placeholder="https://discord.com/channels/123/456" style={{
            background: 'rgba(255,255,255,0.04)',
            border: '1px solid rgba(255,255,255,0.1)',
            borderRadius: 8,
            padding: '0.5rem 0.75rem',
            ...
            <input id="admin-source-name-input" type="text" value={sourceName} onChange={(e) => setSourceName(e.target.value)} placeholder="Nome amigável da source (opcional)" style={{
            background: 'rgba(255,255,255,0.04)',
            border: '1px solid rgba(255,255,255,0.1)',
            borderRadius: 8,
            padding: '0.5rem 0.75rem',
            ...
            <button id="admin-create-source-from-link-btn" onClick={createSourceFromChannelLink} disabled={creatingSource} style={{
        ...
        <section style={{
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.6rem' }}>
            <input id="admin-import-source-id-input" type="text" value={importSourceId} onChange={(e) => setImportSourceId(e.target.value)} placeholder="Source ID (opcional, mas recomendado)" style={{
            flex: 1,
            minWidth: 300,
            background: 'rgba(255,255,255,0.04)',
            border: '1px solid rgba(255,255,255,0.1)',
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.75rem', color: '#cbd5e1' }}>
              <input id="admin-import-dryrun-checkbox" type="checkbox" checked={importDryRun} onChange={(e) => setImportDryRun(e.target.checked)}/>
              Dry-run (simulação)
            </label>
          </div>

          <div id="admin-json-dropzone" onDragOver={(event) => {
            event.preventDefault();
            setDragActive(true);
        }} onDragLeave={() => setDragActive(false)} onDrop={async (event) => {
            event.preventDefault();
            setDragActive(false);
          ...
          {fileName && (<div style={{ marginTop: '0.65rem' }}>
              <p style={{ margin: 0, color: '#93c5fd', fontSize: '0.73rem' }}>Arquivo: {fileName}</p>
              {payloadPreview.map((line) => (<p key={line} style={{ margin: '0.22rem 0 0 0', color: '#cbd5e1', fontSize: '0.72rem' }}>{line}</p>))}
            </div>)}

          {importChunkInfo && importChunkInfo.totalMessages > IMPORT_CHUNK_SIZE && (<p style={{ margin: '0.5rem 0 0 0', color: '#fbbf24', fontSize: '0.71rem' }}>
          ...
          <div style={{ marginTop: '0.7rem' }}>
            <button id="admin-import-json-btn" onClick={importJsonPayload} disabled={importingJson || !parsedPayload} style={{
          ...
          {importProgress && importProgress.total > 1 && (<div style={{ marginTop: '0.7rem' }}>
          ...
          {jsonWasRepaired && !jsonRepairBannerDismissed && (<div style={{
                marginTop: '0.7rem',
              ...
              <button onClick={() => setJsonRepairBannerDismissed(true)} style={{ background: 'none', border: 'none', color: '#fde047', cursor: 'pointer', fontSize: '0.7rem', flexShrink: 0, padding: '0 0.2rem' }}>
                Entendi
              </button>
            </div>)}

          {/* Banner vermelho: JSON irrecuperável */}
          {jsonRepairFailed && (<div style={{
                marginTop: '0.7rem',
