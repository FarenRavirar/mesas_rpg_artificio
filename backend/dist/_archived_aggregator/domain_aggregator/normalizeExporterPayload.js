...
exports.normalizeExporterPayload = void 0;
const pythonParserService_1 = require("../../services/aggregator/pythonParserService");
...
const normalizeMessage = async (item) => {
    ...
    if (content) {
        try {
            ...
            const parsed = await (0, pythonParserService_1.parseMessage)(content, metadata);
            enrichedFields = parsed;
...
const normalizeExporterPayload = async (payload) => {
    ...
    const messages = resolvedMessages.filter((message) => Boolean(message));
...
exports.normalizeExporterPayload = normalizeExporterPayload;
