...
exports.formatForPublication = void 0;
const asDateLabel = (value) => {
    if (!value)
...
const renderOne = (item, index) => {
    ...
    if (draft.style)
        lines.push(`- Estilo: ${draft.style}`);
    if (draft.scheduleText)
        lines.push(`- Agenda: ${draft.scheduleText}`);
    if (draft.slotsText)
        lines.push(`- Vagas: ${draft.slotsText}`);
    if (draft.location || draft.platforms)
        lines.push(`- Local/Plataformas: ${draft.location ?? draft.platforms}`);
    if (draft.ageRating)
        lines.push(`- Classificação: ${draft.ageRating}`);
    if (draft.isPaid) {
    ...
    if (draft.signupText)
        lines.push(`- Contato: ${draft.signupText}`);
    if (draft.synopsis)
        lines.push(`- Resumo: ${draft.synopsis}`);
...
const formatForPublication = (items, date) => {
    const header = [];
...
exports.formatForPublication = formatForPublication;
