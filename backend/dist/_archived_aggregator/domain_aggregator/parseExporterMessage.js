...
exports.parseExporterMessage = void 0;
const classifyPayment_1 = require("./classifyPayment");
const classifySystem_1 = require("./classifySystem");
const extractMediaLinks_1 = require("./extractMediaLinks");
const resolveMasterRecruiter_1 = require("./resolveMasterRecruiter");
const URL_REGEX = /https?:\/\/[^\s)]+/gi;
const cleanValue = (value) => {
    return value.replace(/[*_`~]/g, '').replace(/\s+/g, ' ').trim();
};
const sanitizeExtracted = (value) => {
    if (!value)
        return null;
    const normalized = cleanValue(value);
    return normalized.length > 0 ? normalized : null;
};
const extractHeadingTitle = (content) => {
...
const lineValueByMatchers = (content, matchers) => {
...
const extractSchedule = (content) => {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (!/data\s*[&e]?\s*hor[aá]rios?|data\s*e\s*hora|hor[aá]rios?/i.test(current))
            continue;
...
const extractRawMentions = (message) => {
    ...
    const deduped = new Set([...mentionFromContent, ...mentionFromPayload]);
    return Array.from(deduped.values());
};
const extractExternalLinks = (message) => {
    const links = new Set();
...
const parseExporterMessage = (input) => {
    ...
    const payment = (0, classifyPayment_1.classifyPayment)(message.content);
    const systemClassification = (0, classifySystem_1.classifySystem)(systemText, message.content, knownSystemAliases);
    const rawMentions = extractRawMentions(message);
    const recruiterResolution = (0, resolveMasterRecruiter_1.resolveMasterRecruiter)({
        recruiterName: enriched.recruiterName ?? message.author.nickname ?? message.author.name,
        masterText,
        rawMentions,
    });
    const mediaLinks = (0, extractMediaLinks_1.extractMediaLinks)(message);
    ...
    const confidenceScore = Number(Math.max(0, Math.min(1, structuralConfidence * 0.5
        + payment.confidence * 0.2
        + systemClassification.confidence * 0.25
        + (recruiterResolution.isAmbiguous ? 0.0 : 0.05))).toFixed(2));
    const needsReview = systemClassification.needsReview
...
exports.parseExporterMessage = parseExporterMessage;
