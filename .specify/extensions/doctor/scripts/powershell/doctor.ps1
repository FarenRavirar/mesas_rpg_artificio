...
Check-Directory ".specify/features" ".specify/features/ directory" "error"
Check-Directory ".specify/memory" ".specify/memory/ directory" "error"
Check-Directory ".specify/templates" ".specify/templates/ directory" "warning"
...
$featuresDir = Join-Path $ProjectRoot ".specify/features"
if (Test-Path $featuresDir -PathType Container) {
    $featureDirs = Get-ChildItem $featuresDir -Directory | Sort-Object Name
    if ($featureDirs.Count -eq 0) {
        Write-Host "  [NOTE] No feature directories in .specify/features"
    }
    else {
    ...
    Write-Host "  [NOTE] No .specify/features/ directory"
}
