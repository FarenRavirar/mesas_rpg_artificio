...
13. **Update Project State**:
    - Load `.specify/memory/project-state.md`
    - Update the following sections:
      - **Última atualização**: current timestamp
      - **Estado Técnico Atual**: add entry for completed session/feature
      - **Features Ativas**: update task completion % for the feature
      - **Próxima Ação**: update based on retrospective findings
    - Write updated content back to `.specify/memory/project-state.md`

14. **Update Session Log**:
    - Ensure `.specify/memory/session-log.md` exists (create if missing with header)
    - Append new entry with format:
      ...
      **Decisões arquiteturais:** {key_decisions}
      **Phantom completions detectadas:** {phantom_count}
      **Relatório:** [retro-{timestamp}.md](file:///{FEATURE_DIR}/retros/retro-{timestamp}.md)
      ```

15. **Detect Phantom Completions**:
    - For each task marked `[x]` in `tasks.md`, verify implementation evidence:
      - Check git history for related commits
      - Check for modified files mentioned in task description
      - Check for test files or test evidence
    - Flag tasks marked complete without clear implementation evidence
    - Add phantom completion warnings to session log entry

16. **Offer Constitution Update**:
