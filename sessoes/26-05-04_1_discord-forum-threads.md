...
- 2026-05-06: Mantenedor reforcou que os testes precisam ser diretos no banco de dados e reportou UI ainda distante do usavel: lista mostra 100 pendentes, conteudo aparece como `Post sem texto no corpo`, e nao ha evidencia de drafts gerados. Acao: validar diretamente no DB dev/Beta a janela real, estados e saida de parse antes de deploy.
- 2026-05-06: Mantenedor ativou Message Content Intent e solicitou atualizar documentacao a partir das referencias oficiais Discord, alem de nao parar antes de testes funcionais back->front. Escopo atual: diagnosticar e codar localmente, sem deploy e sem commit; validar API Discord real, parser local, contrato backend/frontend e documentacao SDD antes de qualquer proxima autorizacao.
- 2026-05-06: Teste direto API Discord apos Message Content Intent: 5 posts reais do Covil retornaram corpo pela API (`content_length` 2125, 1878, 1826, 2748, 1293). Primeiro dry-run local revelou bug de match de sistema (`Tormenta20` sem ID e `Planescape` casando `Gamma World` via alias generico). Parser corrigido para preferir nomes especificos, ignorar aliases curtos genericos e aceitar sufixo numerico de versao; teste unitario ampliado.
- 2026-05-06: Re-teste local com conteudo real da API e lista real de sistemas do banco Beta: 5/5 posts parseados como `ready`, `missing=[]`; Forgotten Realms -> D&D 2024, Deicidio -> D&D, Tormenta20 -> Tormenta, Planescape -> D&D, Mage one-shot -> Mage. Documentacao SDD/API atualizada com Message Content Intent, diagnostico direto e fluxo funcional forum -> mensagem -> draft -> mesa draft.
- 2026-05-06: Consulta read-only no banco Beta confirmou que os 5 registros ja ingeridos continuam com `content_raw` tamanho 0 e status `parsed`; drafts atuais estao `needs_review` (170). Conclusao: o banco esta com dados antigos hidratados antes do intent; corrigir a UI em Beta exige reidratacao mutavel apos deploy autorizado, mas a etapa atual permanece local/read-only conforme pedido do mantenedor.
- 2026-05-06: Mantenedor autorizou reidratacao e testes aleatorios. Snapshot antes da escrita: ultimos 7 dias em posts de forum no Beta possuem 11 mensagens, 11 com `content_raw` vazio e 0 preenchidas; drafts existentes seguem `needs_review` (170). Proximo: rebuscar corpos pela API Discord e atualizar somente mensagens alvo, depois testar DB/API/parser por amostragem.
- 2026-05-06: Reidratacao Beta executada para ultimos 7 dias: 11/11 mensagens rebuscadas pela API Discord; 10 starters passaram a ter `content_raw` preenchido (1293-2748 chars) e 1 reply nao-starter com PDF foi identificado como ruido e marcado `ignored`. Os 10 starters ficaram `pending` para reprocessamento.
- 2026-05-06: Teste aleatorio no parser atualmente deployado no Beta mostrou que ele ainda erra preco/vagas/sistema e gera `needs_review`; portanto o problema nao e mais falta de dados, e sim codigo ainda nao deployado. Parser local corrigido reprocessou os 10 starters com sistemas reais do banco Beta: 10/10 `ready`, `missing=[]`. Ajuste adicional local: contatos por mencao de canal Discord (`<#...>`) agora contam como contato valido.
- 2026-05-06: Investigacao fechada do caso Fundacao 0. O PDF era reply nao-starter (`discord_message_id != discord_thread_id`) e nao deve ser considerado post/anuncio. O starter real `1498453714988433519` estava fora da janela movel de 7 dias por data absoluta e foi reidratado manualmente via API Discord: 1632 chars + 1 imagem. Parser local processa 11 starters reais: 10 `ready` e 1 `needs_review` apenas porque `One Two Six (Sistema Inedito)` nao existe no cadastro de sistemas.
- 2026-05-06: Mantenedor reportou que sugestoes automaticas de sistema estavam poluidas por cenario/titulo (Forgotten Realms, Waterdeep, Vecna etc.), pediu que sistema inedito gere sugestao automatica real, que a gestao permita selecionar/selecionar todas/descartar sugestoes e que rejeitar sem motivo seja permitido. Patch local: parser prioriza `Sistema:` do corpo quando ha conteudo; titulo da thread so e fallback sem corpo; sistema explicito sem `system_id` fica `needs_review` e cria `system_suggestion`; rejeicao admin aceita `{}`; GestaoPage ganhou checkboxes, selecionar todas pendentes e descarte em lote. Docs e changelog atualizados antes do deploy autorizado.
- 2026-05-06: Validacao e deploy dev concluidos. Evidencias locais: parser tests 8/8 GREEN, backend build GREEN, frontend build GREEN, changelog JSON OK, `git diff --check` sem erros. Commit `68c379d fix(discord): resolve systems from forum body` enviado para `origin/dev`. Deploy Beta run `25459002977` GREEN (`validate`, `lint`, `enforce-dir`, `migrate`, `deploy-app`, `smoke`) e CodeQL run `25459001718` GREEN. Health Beta `/api/v1/health` HTTP 200 (`status=ok`, `environment=beta`, `db=connected`). Pos-deploy: 11 starters reais reprocessados no banco Beta; 11 drafts `ready`, `missing=[]`, sistemas resolvidos pelo corpo (`Forgotten Realms` -> D&D 2024, `Waterdeep` -> D&D 5e, `Fundacao 0` -> One Two Six).
