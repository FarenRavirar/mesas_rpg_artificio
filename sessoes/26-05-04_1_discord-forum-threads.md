...
- 2026-05-05: Mantenedor pediu ler ultima sessao e fazer deploy dela para `dev`. Sessao lida; branch atual `feat/015-discord-draft-pipeline` com patch pos-deploy ainda nao commitado. Proximo: validar diff, stage explicito, commit na branch, push para `origin/dev`, acompanhar Deploy Beta e health.- 2026-05-06: Mantenedor reforcou que os testes precisam ser diretos no banco de dados e reportou UI ainda distante do usavel: lista mostra 100 pendentes, conteudo aparece como `Post sem texto no corpo`, e nao ha evidencia de drafts gerados. Acao: validar diretamente no DB dev/Beta a janela real, estados e saida de parse antes de deploy.
